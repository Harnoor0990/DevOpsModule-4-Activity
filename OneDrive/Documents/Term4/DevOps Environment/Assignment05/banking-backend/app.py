...
def dashboard():
    ...
    if 'user_email' in session:
        transactions = list(mongo.db.transactions.find({'email': session['user_email']}).sort('timestamp', -1))
        ...
        for trans in transactions:
            ...
            transactions_by_month[month_year].append({
                'type': trans['type'],
                'amount': trans['amount'],
                'timestamp': trans['timestamp'].strftime('%m/%d/%Y, %I:%M:%S %p')
            })
    
    return render_template('dashboard.html', user=user, transactions_by_month=transactions_by_month)
